...
let sources=[],candidates=[],busy=false;
...
function renderSources(){
  ...
  if(!sources.length){
    host.innerHTML='<div class="curator-empty"><strong>No sources yet.</strong><span>Add the first trusted YouTube channel above.</span></div>';
    return;
  }
...
function candidateMatches(item){
  ...
  if(query){
    const hay=[item.title,item.channelTitle,item.sourceLabel,item.description,item.contentPillar,item.show].join(" ").toLowerCase();
...
function renderCandidates(){
  ...
  sourceSelect.innerHTML='<option value="all">All sources</option>'+sources.map(source=>'<option value="'+safe(source.id)+'">'+safe(source.label||source.channelTitle)+'</option>').join("");
  ...
  if(!visible.length){
    host.innerHTML='<div class="curator-empty"><strong>No matching candidates.</strong><span>Sync active sources or change the filters.</span></div>';
    return;
  }
  host.innerHTML=visible.map(item=>{
    ...
    return '<article class="candidate-card">'+
      ...
      '<div class="candidate-copy"><small>'+safe(item.sourceLabel||"Curated source")+' · '+safe(fmt(item.publishedAt))+'</small>'+
...
async function load(message=""){
  try{
    ...
    renderSources();
    renderCandidates();
    if(state.configured)status("Curator ready. External videos remain on YouTube and require SpeakOut editorial review before public release.","ok");
    else status("Curator UI is ready, but the Worker still needs the encrypted YOUTUBE_API_KEY secret before channel sync can run.","warn");
  }catch(error){
    console.error(error);
...
$("syncAll").addEventListener("click",async()=>{
  ...
  try{
    status("Checking all active YouTube sources…","warn");
    ...
    await load();
    status("Sync complete: "+created+" new candidate"+(created===1?"":"s")+(errors?" · "+errors+" source error"+(errors===1?"":"s"):"")+".",errors?"warn":"ok");
...
document.addEventListener("click",async event=>{
  ...
  if(sync){
    setBusy(true);
    try{status("Checking this source…","warn");await adminApi.syncTvCurator(sync.dataset.sourceSync);await load()}
