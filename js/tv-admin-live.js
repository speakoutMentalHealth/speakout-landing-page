    ...
    try {
      ...
      if (!attention.length) {
        ...
        target.innerHTML = '<div class="attention-list">'+attention.slice(0,8).map(({item,issues}) =>
          '<article><div><strong>'+String(item.title || "Untitled").replace(/[&<>"']/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]))+'</strong><small>'+
          String(item.show || "SpeakOut TV").replace(/[&<>"']/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]))+'</small></div><span>'+
          issues.join(" · ").replace(/[&<>"']/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]))+'</span></article>'
